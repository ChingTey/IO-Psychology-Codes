...
*I am experimenting to see if I can use ridge/lasso on a categorical outcome. In theory, I can treat Attrition Yes/No as -1 and 1, which will trick the model to predict value in negative or positive value rather than "Yes" or "No". If I set 'No' as negative, 0 will be our cutoff of determine if the predicted value is Yes (positive value) or No (negative value)*

Let's try recode Attrition
```{r}
Attrition_Data$Attrition <- as.factor(Attrition_Data$Attrition)
levels(Attrition_Data$Attrition)[match("Yes", levels(Attrition_Data$Attrition))] <- "1"
levels(Attrition_Data$Attrition)[match("No", levels(Attrition_Data$Attrition))] <- "-1"

Attrition_Data$Attrition <- as.numeric(Attrition_Data$Attrition)
```


Strangely, the system is coding `No` as 2. This doesn't change the operation, we will use 1.5 as cutoff then.
```{r}
str(Attrition_Data)
```
...
*Let's try using `Attrition` as outcome*
...
```{r}
Attrition_split <- initial_split(Attrition_Data, strata = "Attrition")
...
Select_best will do the work for us in finding the best RMSE based penalty
```{r}
best_penalty <- select_best(tune_res, metric = "rmse")
best_penalty
```

Now we can feed this penalty into the final model.
```{r}
...
# Conclusion
Lasso is 0.085 and Ridge is 0.222. The difference seems to be big.
...
Lasso is not as good as ridge in predicting Attrition. 


## Bonus
This file has been upload to GitHub - https://github.com/ChingTey/IO-Psychology-Codes/tree/Ridge-vs-Lasso 
